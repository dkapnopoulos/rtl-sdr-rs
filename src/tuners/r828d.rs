...
impl R828D {
    pub fn new(_handle: &mut Device) -> R828D {
        let tuner = R828D {
            info: TUNER_INFO,
            regs: REG_INIT,
            freq: 0,
...
impl Tuner for R828D {
    ...
    fn set_freq(&mut self, handle: &Device, freq: u32) -> Result<()> {
        ...
        // Some extra stuff for the 828D tuner
        let air_cable1_in = if freq > 345_000_000 {
            0x00
        } else {
            0x60
        };
...
impl R828D {
    ...
    fn set_pll(&mut self, handle: &Device, freq: u32) -> Result<()> {
        ...
        // TODO(DONE): if chip is R828D set vco_power_ref = 1
        let vco_power_ref = 1;
    ...
    fn write_regs(&mut self, handle: &Device, reg: usize, val: &[u8]) -> Result<()> {
        ...
        loop {
            ...
            handle.i2c_write(R828D_I2C_ADDR, &buf)?;
            val_index += size;
            reg_index += size;
            len -= size;
            if len <= 0 {
    ...
    fn read_reg(&self, handle: &Device, reg: usize, buf: &mut [u8], len: u8) -> Result<()> {
        ...
        handle.i2c_write(R828D_I2C_ADDR, &[reg as u8])?;
        handle.i2c_read(R828D_I2C_ADDR, buf, len)?;
